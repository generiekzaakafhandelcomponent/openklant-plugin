...
@Component({
    selector: 'openklant-create-partij',
    standalone: true,
    imports: [
        AsyncPipe,
        FormModule,
    ...
    templateUrl: './create-partij.component.html'
})
export class CreatePartijComponent implements FunctionConfigurationComponent, OnInit, OnDestroy {
    @Input() save$: Observable<void>;
    @Input() disabled$: Observable<boolean>;
    @Input() pluginId: string;
    @Input() prefillConfiguration$?: Observable<CreatePartijConfig>;
    @Output() valid: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() configuration: EventEmitter<FunctionConfigurationData> = new EventEmitter<FunctionConfigurationData>();

    private saveSubscription!: Subscription;

    private readonly config$ = new BehaviorSubject<CreatePartijConfig | null>(null);
    ...
    formValueChange(formOutput: FormOutput): void {
        const config = formOutput as CreatePartijConfig;
        this.config$.next(config);
        this.handleValid(config);
    }

    private handleValid(formOutput: CreatePartijConfig): void {
        const valid = 
            !!formOutput.soortPartij &&
            !!formOutput.objectId &&
